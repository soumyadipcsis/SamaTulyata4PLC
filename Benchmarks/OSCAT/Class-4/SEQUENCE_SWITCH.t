...
STEP 'SEQUENCE_RUNNING'
    ...
    IF RUN AND _STEP = 1 THEN
        IF NOT Q1 AND IN1 THEN
            Q0 := FALSE;
            Q1 := TRUE;
            last := tx;
        ELSIF NOT Q1 AND tx - last > 1000 THEN
            STATUS := BYTE#2;
            Q0 := FALSE;
            RUN := FALSE;
        ELSIF Q1 AND tx - last >= 1000 THEN
            _STEP := 2;
            last := tx;
        END
    END
    ...
    IF RUN AND _STEP = 2 THEN
        IF NOT Q2 AND IN2 THEN
            Q1 := FALSE;
            Q2 := TRUE;
            last := tx;
        ELSIF NOT Q2 AND tx - last > 1000 THEN
            STATUS := BYTE#3;
            Q1 := FALSE;
            RUN := FALSE;
        ELSIF Q2 AND tx - last >= 1000 THEN
            _STEP := 3;
            last := tx;
        END
    END
