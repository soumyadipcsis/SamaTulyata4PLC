...
STEP 'PAUSE_RESUME'
    (* Pause or resume the sequence based on input *)
    IF PAUSE AND RUN THEN
        RUN := FALSE;  (* Pause sequence *)
        STATUS := BYTE#120;  (* Pause status *)
    ELSIF NOT PAUSE AND NOT RUN THEN
        RUN := TRUE;   (* Resume sequence *)
        STATUS := BYTE#121;  (* Resume status *)
    END
ENDSTEP
